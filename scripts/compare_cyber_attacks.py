...
def run_simulation(config_file, description, timeout=120):
    ...
    try:
        ...
        print(f"  Impeachments: {metrics['impeachments']}")
...
def main():
    ...
    if coverage_diff > 10:
        ...
        print(f"     Baseline performs better even under attacks ({coverage_diff:.1f}%)")
    ...
    if reip_impeachments > 0:
        ...
        print(f"     No impeachments - governance may not be triggering")
    
    if reip_loop_detections > 100:
        print(f"     WARNING: {reip_loop_detections} loop detections may indicate false positives!")
