...
def run_comparison():
    ...
    if coverage_diff > 2:
        ...
        print(f"     Baseline achieves SIGNIFICANTLY BETTER coverage ({coverage_diff:.1f}%)")
    else:
    ...
    if baseline_results['exploration_efficiency'] > 0.001:
        ...
        if efficiency_ratio > 1.1:
            ...
            print(f"     Baseline is {1/efficiency_ratio:.1f}x MORE EFFICIENT")
    else:
        print(f"     Cannot compute ratio (baseline efficiency too low)")
    ...
    if coverage_diff > 5:
        ...
        print(f"   1.   Baseline explores better: REIP is {abs(coverage_diff):.1f}% behind")
    else:
        print(f"   1. ℹ  Both systems have limited exploration")
    
    if abs(time_diff) > 10:
        faster = "REIP" if time_diff > 0 else "Baseline"
        print(f"   2. {'' if time_diff > 0 else ''} {faster} is faster by {abs(time_diff):.1f}s")
    else:
    ...
    if reip_results['completed'] and not baseline_results['completed']:
        ...
        print(f"   3.   Baseline completes, REIP does not")
    else:
    ...
    if coverage_diff > 5 and efficiency_ratio > 1.05:
        ...
        print("     BASELINE IS BETTER FOR PURE EXPLORATION")
