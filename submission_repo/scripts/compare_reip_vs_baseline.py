...
def run_experiment(cfg_path, runs=5, parallel=True, max_workers=None):
    ...
    if max_coverage > 1.0:
        print(f"    WARNING: Found coverage value > 100%: {max_coverage:.2%}")
...
def plot_comparison(baseline_clean, baseline_adv, reip_clean, reip_adv, output='comparison.png'):
    ...
    print(f"\nOK Comparison plot saved to: {output}")
...
def main():
    ...
    if args.parallel:
        ...
        print("  Parallel execution disabled (sequential mode)")
    ...
    if args.export_csv:
        ...
        with csv_path.open('w', newline='') as csvfile:
            ...
            for row in summary_rows:
                writer.writerow(row)
        print(f"\nOK Metrics exported to: {csv_path}")
    ...
    if args.export_histories:
        ...
        def write_histories(tag: str, data: dict):
            ...
            print(f"OK Histories exported for {tag} to: {out_path}")
