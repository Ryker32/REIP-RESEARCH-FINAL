#!/usr/bin/env python3
"""
Alternate threshold figure with overlaid bars so Raft appears on top of REIP.
"""
...
def annotate_overlay(ax, y_vals, reip_counts, raft_counts):
    for y, reip, raft in zip(y_vals, reip_counts, raft_counts):
        ax.text(reip + 1.2, y - 0.16, f"{reip}",
                color=CLR_REIP, fontsize=9.0, fontweight="bold",
                ha="left", va="center")
        ax.text(raft + 1.2, y + 0.16, f"{raft}",
                color=CLR_RAFT, fontsize=9.0, fontweight="bold",
                ha="left", va="center")
...
def main():
    ...
    annotate_overlay(ax, y, reip_counts, raft_counts)

    ax.set_title("Success Under Stricter Coverage Thresholds", fontweight="bold", pad=12)
    ax.text(0.0, 1.01, "100 bad-leader trials", transform=ax.transAxes,
    ...
    for ext in ("png", "pdf"):
        out = OUTPUT_DIR / f"fault_threshold_overlay.{ext}"
